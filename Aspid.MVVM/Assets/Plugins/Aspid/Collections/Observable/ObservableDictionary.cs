        ...
        where TKey : notnull
    {
            ...
            set
            {
                lock (SyncRoot)
                {
                    if (_dictionary.TryGetValue(key, out var oldValue))
                    {
                        ...
                        Invoke(NotifyCollectionChangedEventArgs<KeyValuePair<TKey, TValue>>.Replace(oldItem, newItem, -1));
                    }
                    else
                    {
                        Add(key, value);
        ...
        public void Add(TKey key, TValue value)
        {
            lock (SyncRoot)
            {
                ...
                Invoke(NotifyCollectionChangedEventArgs<KeyValuePair<TKey, TValue>>.Add(new KeyValuePair<TKey, TValue>(key, value), -1));
            }
        }
        ...
        public bool Remove(TKey key)
        {
            lock (SyncRoot)
            {
                ...
                OnRemoved(key, value);
                Invoke(NotifyCollectionChangedEventArgs<KeyValuePair<TKey, TValue>>.Remove(new KeyValuePair<TKey, TValue>(key, value), -1));
                return true;
            }
        }
        ...
        public bool Remove(KeyValuePair<TKey, TValue> item)
        {
            lock (SyncRoot)
            {
                ...
                Invoke(NotifyCollectionChangedEventArgs<KeyValuePair<TKey, TValue>>.Remove(new KeyValuePair<TKey, TValue>(item.Key, value2), -1));
                return true;
            }
        }
...
#pragma warning restore CS8767
        ...
        public void Clear()
        {
            lock (SyncRoot)
            {
                ...
                Invoke(NotifyCollectionChangedEventArgs<KeyValuePair<TKey, TValue>>.Reset());
            }
        }
