...
{
    /// <summary>
    /// MonoBehaviour binder that sets the <see cref="AudioSource.panStereo"/> property on a group of <see cref="AudioSource"/>
    /// components, applying the configured selected or default value to each entry based on the bound
    /// enum ViewModel value. The value is clamped to the range [-1, 1].
    /// </summary>
