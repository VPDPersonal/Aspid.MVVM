...
**Предполагается знание:** [Counter](../01.%20Counter/README.ru.md) — `[OneWayBind]`, `[RelayCommand]`
...
Порядок важен: цепочка применяет звенья сверху вниз, поэтому восклицательный знак останется снаружи
тега цвета.

> Ни строки кода. Раньше этот сэмпл возил с собой собственный `PaintNameConverter` — теперь
> `RichTextColorConverter` поставляется с пакетом, и писать свой не нужно. Полный каталог из 211
> конвертеров — в [Конвертерах](../../Documentation/ru/08-converters.md).

> Текст, который ввёл игрок, — отдельный разговор. TMP исполняет разметку в любой строке: ник
> `<size=400%>` растянет ярлык на экране каждого, кто его увидит. Если поле `Name` доступно другим
> игрокам, первым звеном цепочки должен идти `RichTextSanitizeConverter` или
> `RichTextNoParseConverter`.
...
[Bind Modes →](../03.%20BindModes/README.ru.md) — все четыре режима биндинга на одном экране и свой конвертер.
