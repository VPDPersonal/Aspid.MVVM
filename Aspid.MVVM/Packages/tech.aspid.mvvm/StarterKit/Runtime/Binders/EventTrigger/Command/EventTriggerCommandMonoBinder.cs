...
{
    /// <summary>
    /// <see cref="ComponentMonoBinder{TComponent}"/> that executes a command when the selected
    /// <see cref="EventTrigger"/> event fires.
    /// </summary>
    /// <remarks>
        ...
        IBinder<IRelayCommand<EventTriggerType>>
    {
        ...
        protected override void OnValidate()
        {
            ...
            else if (_dataCommand is not null) OnCanExecuteChanged(_dataCommand);
            else if (_typeCommand is not null) OnCanExecuteChanged(_typeCommand);
        }
    ...
    /// <summary>
    /// <see cref="ComponentMonoBinder{TComponent}"/> that executes a command when the selected
    /// <see cref="EventTrigger"/> event fires with <see cref="Param"/>.
    /// </summary>
    /// <remarks>
    /// Accepts a plain command, one that receives the <see cref="BaseEventData"/>, or one that receives the
    /// <see cref="EventTriggerType"/> as its first argument.
    ...
    public abstract partial class EventTriggerCommandMonoBinder<T> : ComponentMonoBinder<EventTrigger>,
    ...
    /// <summary>
    /// <see cref="ComponentMonoBinder{TComponent}"/> that executes a command when the selected
    /// <see cref="EventTrigger"/> event fires with <see cref="Param1"/>, <see cref="Param2"/>.
    /// </summary>
    /// <remarks>
    /// Accepts a plain command, one that receives the <see cref="BaseEventData"/>, or one that receives the
    /// <see cref="EventTriggerType"/> as its first argument.
    ...
    /// <summary>
    /// <see cref="ComponentMonoBinder{TComponent}"/> that executes a command when the selected
    /// <see cref="EventTrigger"/> event fires with <see cref="Param1"/>, <see cref="Param2"/>, <see cref="Param3"/>.
    /// </summary>
    /// <remarks>
    /// Accepts a plain command, one that receives the <see cref="BaseEventData"/>, or one that receives the
    /// <see cref="EventTriggerType"/> as its first argument.
