...
namespace Aspid.MVVM.StarterKit
{
    ...
    public struct ConverterFallback<T>
    {
        ...
        [field: Tooltip("What to do with a value that will not convert. " +
            "Return Input passes it through when it fits the output type, and otherwise uses the fallback.")]
        ...
        public readonly T Fail(IConverter converter, object? value, string problem) =>
            Fail(Mode, converter, value, problem, FallbackValue);

        /// <summary>
        /// Reports the failure and returns what the specified mode says to. For converters
        /// that serialize the fallback in another shape and build the <typeparamref name="T"/> themselves.
        /// </summary>
        /// <param name="mode">What to do with the value.</param>
        /// <param name="converter">The failing converter — pass <see langword="this"/>.</param>
        /// <param name="value">The value that would not convert.</param>
        /// <param name="problem">What is wrong, as a sentence without the trailing period.</param>
        /// <param name="fallbackValue">Returned instead of the value, mode allowing.</param>
        /// <returns>
        /// The value itself when <paramref name="mode"/> is
        /// <see cref="ConverterFailureMode.ReturnInput"/> and the value already is a
        /// <typeparamref name="T"/>; otherwise, <paramref name="fallbackValue"/>.
        /// </returns>
        internal static T Fail(
            ConverterFailureMode mode,
            IConverter converter,
            object? value,
            string problem,
            T fallbackValue)
        {
            // The runtime check is what makes ReturnInput generic: any converter whose input turns
            // out to fit the output honors the mode, and no converter has to say so itself.
            if (mode is ConverterFailureMode.ReturnInput)
            {
                ...
                var name = ConverterMessageText.GetTypeName(typeof(T));

                converter.LogError(
                    problem: problem,
                    consequence: $"On Failure is Return Input, but the input cannot stand in for {name}; using the fallback.");
