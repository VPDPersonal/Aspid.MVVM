...
namespace Aspid.MVVM.StarterKit
{
    ...
    public sealed class PadStringConverter : IConverter<string?, string?>
    {
        ...
        /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="totalWidth"/> is negative.</exception>
        public PadStringConverter(
            int totalWidth,
            char padChar = ' ',
            bool padLeft = true)
        {
            _padChar = padChar;
            _padLeft = padLeft;
            _totalWidth = totalWidth >= 0 ? totalWidth : throw new ArgumentOutOfRangeException(nameof(totalWidth));
        }
        ...
        public string? Convert(string? value)
        {
            ...
            return _padLeft
                ? value.PadLeft(_totalWidth, _padChar)
                : value.PadRight(_totalWidth, _padChar);
        }
    }
}
