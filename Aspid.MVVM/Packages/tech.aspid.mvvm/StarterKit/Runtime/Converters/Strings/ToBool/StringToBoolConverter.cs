...
namespace Aspid.MVVM.StarterKit
{
    ...
    {
        [Tooltip("The spellings read as true, case-insensitive. The first is written back for true.")]
        [SerializeField] private string[] _trueTokens = { "true", "1", "yes", "on" };

        [Tooltip("The spellings read as false, case-insensitive. Empty: unmatched text takes the fallback quietly.")]
        ...
        /// <param name="trueTokens">The spellings read as <see langword="true"/>; empty keeps the usual ones. The first is written back.</param>
        /// <param name="falseTokens">The spellings read as <see langword="false"/>; empty makes unmatched text take the fallback quietly.</param>
        ...
        /// <returns>Whether the text matches an accepted spelling, or the fallback when it matches none.</returns>
        public bool Convert(string? value)
        {
            ...
            if (_trueTokens is not { Length: > 0 })
            {
                return this.UseFallback(
                    fallback: _fallback,
                    problem: "the list of spellings read as true is empty, so no text can read as true");
            }
            ...
            if (_falseTokens is { Length: > 0 })
            {
                ...
                return this.UseFallback(
                    fallback: _fallback,
                    problem: value.Expected("a boolean spelling"));
            }
        ...
        /// <returns>The first spelling authored for it, or the plain word when none is authored.</returns>
        public string ConvertBack(bool value)
        {
            if (value)
            {
                return _trueTokens is { Length: > 0 }
                    ...
                    : this.UseFallback(
                        fallback: "true",
                        problem: "the list of spellings read as true is empty");
            }
