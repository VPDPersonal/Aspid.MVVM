...
namespace Aspid.MVVM.StarterKit
{
        ...
        IConverter<Vector2, Vector2>, IConverter<Vector3, Vector3>, IConverter<Vector4, Vector4>
    {
        ...
        [Tooltip("Vector the bound one is combined with. Only the components the bound vector carries are read.")]
        ...
        {
            this.LogError(
                problem: $"the operation {_operation.Describe()} is not a declared {nameof(VectorOperation)}",
        ...
        private static Vector4 Divide(Vector4 value, Vector4 operand) => new(
            operand.x is 0f ? value.x : value.x / operand.x,
            operand.y is 0f ? value.y : value.y / operand.y,
            operand.z is 0f ? value.z : value.z / operand.z,
            operand.w is 0f ? value.w : value.w / operand.w);

        // Unity has no Vector4.Reflect; this is the formula its narrower ones use.
