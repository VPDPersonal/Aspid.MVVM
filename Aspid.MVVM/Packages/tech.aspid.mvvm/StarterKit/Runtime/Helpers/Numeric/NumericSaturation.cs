...
namespace Aspid.MVVM.StarterKit
{
    ...
    internal static class NumericSaturation
    {
        ...
        {
            // NaN fails every comparison, so it goes first.
            double.NaN => 0,
            >= int.MaxValue => int.MaxValue,
            <= int.MinValue => int.MinValue,
            _ => (int)value
        ...
        internal static long ToLong(double value) => value switch
        {
            // NaN fails every comparison, so it goes first.
            double.NaN => 0L,
            // 2^63: long.MaxValue itself has no exact double.
        ...
        {
            // NaN and infinities fit in a float; clamping them would change their meaning.
