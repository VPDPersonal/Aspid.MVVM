...
namespace Aspid.MVVM.StarterKit
{
    ...
    /// reused — editing the entries afterwards does not rebuild it.
    /// </remarks>
    [Serializable]
    [TypeSelectorDisplay(
    ...
    public sealed class EnumToDropdownOptionDataConverter : IConverter<Enum, IEnumerable<TMP_Dropdown.OptionData>>
    {
        ...
        /// </param>
        public EnumToDropdownOptionDataConverter(Entry[]? entries, bool useInspectorNames = true)
        {
            // The array is copied so a caller holding on to it cannot rewrite the entries afterwards.
            _entries = entries is null ? Array.Empty<Entry>() : (Entry[])entries.Clone();
        ...
        /// selected is a state and not a mistake.
        /// </returns>
        public IEnumerable<TMP_Dropdown.OptionData> Convert(Enum value)
        {
            ...
            foreach (var name in names)
            {
                ...
                // Built through the properties rather than a constructor: the overload taking text
                // and an image also takes a color in newer TextMeshPro.
        ...
        // Reported ahead of the cache: a bad entry is invisible in the dropdown, and reporting it
        // once would leave a designer opening the scene later with nothing to see.
        ...
        private string LabelFor(Type type, string name, Entry? entry)
        {
        ...
        public struct Entry
        {
            ...
            public Entry(string name, string? label = null, Sprite? icon = null)
            {
                Name = name;
                Icon = icon;
