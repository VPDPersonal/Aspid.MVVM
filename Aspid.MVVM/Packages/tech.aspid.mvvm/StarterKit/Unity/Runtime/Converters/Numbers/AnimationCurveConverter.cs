...
namespace Aspid.MVVM.StarterKit
{
    ...
    public sealed class AnimationCurveConverter : IConverter<float, float>, IConverter<double, double>
    {
        ...
        {
            if (_curve is null || _curve.length == 0)
            {
                this.LogError("no curve is assigned",
        ...
        private float Normalize(float value)
        {
            ...
            this.LogError($"the input range is empty (min and max are both {_inputMin})",
                "Reading the curve at its start.");

            return 0f;
        }

        // AnimationCurve evaluates in float, so the double width runs through it.
