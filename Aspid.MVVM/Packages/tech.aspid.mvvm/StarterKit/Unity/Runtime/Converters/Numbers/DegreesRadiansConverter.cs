...
namespace Aspid.MVVM.StarterKit
{
        ...
        ITwoWayConverter<double, double>
    {
        ...
        /// </summary>
        /// <param name="value">The angle, in degrees — or in radians when inverted.</param>
        /// <returns>The angle, in radians — or in degrees when inverted.</returns>
        public float Convert(float value) => value * (_isInvert ? Mathf.Rad2Deg : Mathf.Deg2Rad);

        /// <summary>
        /// Converts the specified angle back in the opposite direction.
        /// </summary>
        /// <param name="value">The angle, in radians — or in degrees when inverted.</param>
        /// <returns>The angle, in degrees — or in radians when inverted.</returns>
        public float ConvertBack(float value) => value * (_isInvert ? Mathf.Deg2Rad : Mathf.Rad2Deg);

        // Unity's own constants, so the double width runs through the float factor.
