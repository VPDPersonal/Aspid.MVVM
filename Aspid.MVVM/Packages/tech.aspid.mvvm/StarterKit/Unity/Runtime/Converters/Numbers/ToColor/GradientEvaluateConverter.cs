...
namespace Aspid.MVVM.StarterKit
{
    ...
    public sealed class GradientEvaluateConverter : IConverter<float, Color>, IConverter<double, Color>
    {
        ...
        /// <param name="gradient">
        /// The gradient the value is read from. Without one, the failure is reported as an error and
        /// white is returned.
        /// </param>
        /// <param name="inputMin">
        ...
        public Color Convert(float value)
        {
            ...
            {
                this.LogError("no gradient is assigned",
                    "Returning white.");
        ...
        private float Normalize(float value)
        {
            ...
            if (span != 0f) return Mathf.Clamp01((value - _inputMin) / span);

            this.LogError($"the input range is empty (min and max are both {_inputMin})",
                "Reading the gradient at its start.");

            return 0f;
        }

        // Gradient evaluates in float, so the double width runs through it.
