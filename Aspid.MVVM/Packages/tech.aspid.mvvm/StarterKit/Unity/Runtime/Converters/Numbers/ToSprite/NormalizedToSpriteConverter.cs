...
namespace Aspid.MVVM.StarterKit
{
    ...
    public sealed class NormalizedToSpriteConverter : IConverter<float, Sprite?>, IConverter<double, Sprite?>
    {
        ...
        public Sprite? Convert(float value)
        {
            ...
            {
                this.LogError("no frames are assigned",
                    "Returning null.");

                return null;
            }

            // An amount of exactly 1 floors one past the last frame, so the index is clamped, not the amount.
            var index = Mathf.Clamp(Mathf.FloorToInt(Mathf.Clamp01(value) * _frames.Length), 0, _frames.Length - 1);
            return _frames[index];
        }

        // The amount only picks a frame, so a float's precision is more than the pick can use.
