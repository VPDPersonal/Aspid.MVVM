...
namespace Aspid.MVVM.StarterKit
{
        ...
        IConverter<double, string>
    {
        ...
        [Tooltip("Writes the number itself. With the slot cleared, the number is written in the " +
            "general format.")]
        [TypeSelector]
        ...
        private string Format(float value) => _number?.Convert(value) ?? value.ToString(CultureInfo.CurrentCulture);
        ...
        private Color ColorFor(float value)
        {
            ...
            // The stops are authored in any order, so the highest qualifying threshold has to be found.
        ...
        string IConverter<double, string>.Convert(double value) => Convert(NumericSaturation.ToFloat(value));
    }
}
