...
namespace Aspid.MVVM.StarterKit
{
    ...
    {
        // The field keeps the name _values although its type is now Mode: renaming a serialized
        // field orphans the prefab-instance overrides authored against the old path, and
        // [FormerlySerializedAs] does not reach them.
        ...
        /// </param>
        public Vector2Vector3Converter(Mode mode, float thirdValue = 0)
        {
            _values = mode;
        ...
        /// </returns>
        public Vector2 ConvertBack(Vector3 value) => _values switch
        {
        ...
        // The same mapping read from the other side, so a binder that starts from the 3D value picks
        // this converter rather than a mirror of it.
        Vector2 IConverter<Vector3, Vector2>.Convert(Vector3 value) => ConvertBack(value);

        Vector3 ITwoWayConverter<Vector3, Vector2>.ConvertBack(Vector2 value) => Convert(value);

        private Vector2 UndeclaredBack()
        {
            this.LogError(
                $"the mode {_values.Describe()} is not a declared {nameof(Mode)}",
                "Returning a zero vector.");

            return default;
        }

        private Vector3 Undeclared()
        {
            this.LogError(
                $"the mode {_values.Describe()} is not a declared {nameof(Mode)}",
                "Returning a zero vector.");
        ...
        /// </summary>
        /// <remarks>
        /// The members are appended rather than filed in alphabetical order because the declaration
        /// order is the value Unity stores — inserting YX between XZ and YZ would silently repoint
        /// every field authored as YZ.
        /// </remarks>
        public enum Mode
        {
            /// <summary>
