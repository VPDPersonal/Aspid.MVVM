...
namespace Aspid.MVVM.StarterKit
{
        ...
        ITwoWayConverter<Vector4, Vector3>
    {
        ...
        // The same mapping read from the other side, so a binder that starts from the wide value picks
        // this converter rather than a mirror of it.
        Vector3 IConverter<Vector4, Vector3>.Convert(Vector4 value) => ConvertBack(value);

        Vector4 ITwoWayConverter<Vector4, Vector3>.ConvertBack(Vector3 value) => Convert(value);

        private Vector3 Undeclared(Vector4 value)
        {
            this.LogError(
                $"the component {_drop.Describe()} is not a declared {nameof(Vector4Component)}",
                "Dropping W.");
