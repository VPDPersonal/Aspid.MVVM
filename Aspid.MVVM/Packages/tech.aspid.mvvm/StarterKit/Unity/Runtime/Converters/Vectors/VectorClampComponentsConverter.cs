...
namespace Aspid.MVVM.StarterKit
{
    ...
    {
        [Tooltip("The lowest each axis is allowed to be. Only the components the bound vector carries " +
            "are read.")]
        [SerializeField] private Vector4 _min = new(-1f, -1f, -1f, -1f);

        [Tooltip("The highest each axis is allowed to be. Only the components the bound vector carries " +
            "are read.")]
        ...
        // A bound on an axis the binding never reads cannot be wrong about it, so only the axes it
        // carries are checked.
        private void ReportInvertedAxes(int axes)
        {
            ...
            this.LogError($"the minimum {_min} is above the maximum {_max} on at least one axis",
                "Clamping those axes to the swapped bounds.");
        }
