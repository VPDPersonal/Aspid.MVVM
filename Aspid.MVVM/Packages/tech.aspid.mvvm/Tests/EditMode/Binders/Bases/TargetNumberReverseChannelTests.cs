...
{
    /// <summary>
    /// Regression tests for the <see cref="BindMode.OneWayToSource"/> channel of
    /// <see cref="TargetFloatBinder{TTarget}"/> and <see cref="TargetIntBinder{TTarget}"/>.
    /// </summary>
    /// <remarks>
    /// The property's own type reaches the inherited <c>ValueChanged</c> event: a class member always wins over a
    /// default interface implementation, so <see cref="INumberReverseBinder"/>'s bridge does not apply there. These
    /// tests pin every combination of binder and ViewModel field type so that native channel cannot go silent.
    /// </remarks>
    [TestFixture]
