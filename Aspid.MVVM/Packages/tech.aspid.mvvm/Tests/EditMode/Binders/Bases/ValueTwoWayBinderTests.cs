...
{
    /// <summary>
    /// Tests for <see cref="ValueTwoWayBinder{T}"/> that are not already covered by <see cref="ValueBinderFeedbackTests"/>.
    /// </summary>
    [TestFixture]
    public sealed class ValueTwoWayBinderTests
    {
        [Test]
        public void Ctor_NoneMode_Throws() =>
            Assert.Throws<ArgumentException>(() => new ValueTwoWayBinder<int>(0, BindMode.None));

        [Test]
        public void DefaultCtor_StoresTheDefaultValueAndTwoWayMode()
        {
            var binder = new ValueTwoWayBinder<int>();
        ...
        {
            var binder = new ValueTwoWayBinder<int>(0, new DoublingConverter());
        ...
        {
            var binder = new ValueTwoWayBinder<int>(9);
