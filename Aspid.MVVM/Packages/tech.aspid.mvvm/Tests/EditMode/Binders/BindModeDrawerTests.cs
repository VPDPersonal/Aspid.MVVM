...
namespace Aspid.MVVM.Tests
{
    ...
    /// </remarks>
    [TestFixture]
    public sealed class BindModeDrawerTests
    {
        ...
        public void EveryBindersOverride_IsReachableFromItsType()
        {
            ...
            Assert.IsNotNull(attribute, "У биндера не нашлось BindModeOverride, хотя база его объявляет");
            Assert.IsTrue(attribute.IsOne || attribute.Modes.Contains(BindMode.OneWay),
                "Разрешённые режимы биндера не включают OneWay");
        }
        ...
        public void TheModeField_IsWritableThroughItsSerializedProperty()
        {
            ...
            Assert.IsNotNull(property, "У биндера нет сериализуемого поля _mode");

            property.enumValueIndex = (int)BindMode.OneTime;
            serializedObject.ApplyModifiedPropertiesWithoutUndo();

            Assert.AreEqual(BindMode.OneTime, binder.Mode, "Запись через SerializedProperty не дошла до биндера");
        }

        /// <summary>
        /// A binder that allows only the one-way modes must not be silently holding a two-way one after a scene is
        /// loaded — the drawer's deferred correction is what fixes that, and the default mode is what it corrects to.
        /// </summary>
        [Test]
        public void ABindersDefaultMode_IsOneItsOwnOverrideAllows()
        {
            var binder = NewBinder<ScrollbarSizeMonoBinder>();
            var attribute = AttributeOf(binder);

            var allowed = attribute.IsAll
                ? new[] { BindMode.OneWay, BindMode.OneTime, BindMode.TwoWay, BindMode.OneWayToSource }
                : attribute.Modes.Concat(attribute.IsOne
                    ? new[] { BindMode.OneWay, BindMode.OneTime }
                    : System.Array.Empty<BindMode>()).ToArray();

            Assert.Contains(binder.Mode, allowed, "Режим по умолчанию не входит в разрешённые классом");
        }
        ...
        private T NewBinder<T>()
            where T : MonoBinder
        {
            var gameObject = new GameObject("Binder");
