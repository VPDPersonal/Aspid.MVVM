...
namespace Aspid.MVVM.Tests
{
    ...
    /// </summary>
    /// <remarks>
    /// A binder is only ever handed a value through its interface — <c>IBindableMember.Add</c> pattern-matches on
    /// <see cref="IBinder{T}"/> and throws <c>BinderInvalidCastException</c> when the match fails. An overload the
    /// type does not expose through the matching interface is therefore dead: it compiles, it reads as supported,
    /// and it throws at <c>Bind</c>. <c>SliderCommandBinder</c> shipped four such overloads per arity.
    /// <para/>
    ...
    public sealed class BinderInterfaceCoverageTests
    {
        ...
        public void EverySetValueOverload_IsReachableThroughIBinder()
        {
            ...
            Assert.IsEmpty(
                unreachable,
                "Перегрузки SetValue, недостижимые через IBinder<T> — при Bind они дают BinderInvalidCastException:"
                + Environment.NewLine
