...
{
    /// <summary>
    /// Tests for <see cref="UnityDelegateOneWayToSourceBinder{T}"/> and <see cref="UnityDelegateOneWayToSourceBinder{TTarget,T}"/>.
    /// </summary>
    [TestFixture]
    public sealed class UnityDelegateOneWayToSourceBinderTests
    {
        [Test]
        public void SubscribeCtor_InvokingTheCallback_RaisesValueChanged()
        {
            UnityAction<int>? callback = null;
            var binder = new UnityDelegateOneWayToSourceBinder<int>(subscribe: c => callback = c);
        ...
        public void FactoryCtor_BothFactoriesNull_Throws() =>
            Assert.Throws<ArgumentException>(() => new UnityDelegateOneWayToSourceBinder<int>());

        [Test]
        public void FactoryCtor_GetValueOnBound_IsPushedOnBind()
        {
            var binder = new UnityDelegateOneWayToSourceBinder<int>(getValueOnBound: () => 3);
        ...
        {
            var binder = new UnityDelegateOneWayToSourceBinder<int>(getValueOnUnbinding: () => 8);
            var member = new OneWayToSourceBindableMember<int>(_ => { });
        ...
        public void TargetSubscribeCtor_NullTarget_Throws() =>
            Assert.Throws<ArgumentNullException>(
                () => new UnityDelegateOneWayToSourceBinder<object, int>(null, (UnityAction<object, UnityAction<int>>)null));
        ...
        public void TargetSubscribeCtor_ReceivesTheTargetAndTheCallback()
        {
            ...
            UnityAction<int>? callback = null;
            var binder = new UnityDelegateOneWayToSourceBinder<object, int>(target, (t, c) =>
            {
                receivedTarget = t;
                callback = c;
            });
        ...
        public void TargetFactoryCtor_NullTarget_Throws() =>
            Assert.Throws<ArgumentNullException>(
                () => new UnityDelegateOneWayToSourceBinder<object, int>(null, getValueOnBound: _ => 1));

        [Test]
        public void TargetFactoryCtor_BothFactoriesNull_Throws() =>
            Assert.Throws<ArgumentException>(() => new UnityDelegateOneWayToSourceBinder<object, int>(new object()));
        ...
        public void TargetFactoryCtor_GetValueOnBound_IsPushedWithTheTargetOnBind()
        {
            var target = new object();
            object? receivedTarget = null;
            var binder = new UnityDelegateOneWayToSourceBinder<object, int>(target, getValueOnBound: t =>
            {
                receivedTarget = t;
                return 6;
            });
