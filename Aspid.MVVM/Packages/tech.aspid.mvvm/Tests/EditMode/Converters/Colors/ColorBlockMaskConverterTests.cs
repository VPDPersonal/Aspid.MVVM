...
namespace Aspid.MVVM.Tests
{
    ...
    /// and <see cref="ColorBlockStateConverter"/> — which of the five state colors each writes, and
    /// that the four it does not write survive bit for bit.
    /// </summary>
    /// <remarks>
    /// Source blocks never start from <see cref="ColorBlock.defaultColorBlock"/>: it gives highlighted and selected the same colour,
    /// which would hide a mask reading the wrong field. <see cref="SelectableStates.All"/> must stay the default in both the constructor and the field initializer.
    /// </remarks>
    [TestFixture]
    public sealed class ColorBlockMaskConverterTests
    {
        private const float Tolerance = 1e-6f;

        /// <summary>The five single-flag states, in the order <c>Convert</c> writes them.</summary>
        private static readonly SelectableStates[] _everyState =
        {
        ...
        /// <summary>Half gray at quarter alpha: every channel is a power of two, so a multiply is exact.</summary>
        private static readonly Color _tint = new(0.5f, 0.5f, 0.5f, 0.25f);

        /// <summary>The color <see cref="ColorBlockStateConverter"/> writes — not gray, not any authored state.</summary>
        private static readonly Color _written = new(0.25f, 0.75f, 0.125f, 0.375f);
        ...
        public void TintConverter_DefaultMask_MultipliesEveryStateIncludingDisabled(
            ...
            float a)
        {
            var result = new ColorBlockTintConverter(_tint).Convert(Authored());
        ...
        // Same tint that changes all five under All, so this pins the mask rather than the color.
        [Test]
        ...
        // The Lerp amount defaults to one, so a converter constructed without it travels the whole
        // way — including to the tint's own alpha, which is what separates it from Replace.
        [Test]
        ...
        // An undeclared blend is reported and the color passes through untinted, and the mask decides
        // whether it is reported at all — the states are filtered before the blend runs, not after. A
        // converter that blended first and filtered second would report for both masks. The report
        // names the block converter rather than ColorTintConverter, whose helper does the blending.
        [Test]
        public void TintConverter_UndeclaredBlend_ReportsItOnlyForStatesInsideTheMask()
        {
            ...
            var untouched = new ColorBlockTintConverter(_tint, undeclared, SelectableStates.None).Convert(Authored());
            AssertSameColor(Authored().pressedColor, untouched.pressedColor, "Pressed, outside the mask");
        }
        ...
        // Disabled plus gray, so picking it in the Inspector already recolors a state before
        ...
        public void StateConverter_ParameterlessConstructor_WritesGrayIntoDisabledOnly()
        {
            ...
            Assert.AreEqual(Color.gray, result.disabledColor, "The default converter should write gray into Disabled.");
        ...
        {
            var result = new ColorBlockStateConverter(state, _written).Convert(Authored());

            Assert.AreEqual(_written, ColorOf(result, state), $"{state} sits inside the mask and should carry the authored color.");
            AssertUntouchedOutside(state, result);
        }

        // "This toggle stays lit once chosen": one converter pinning two states to one color is the
        // reason the field is a mask rather than a single choice.
        [Test]
        public void StateConverter_CombinedMask_PinsBothStatesToTheSameColor()
        {
            const SelectableStates mask = SelectableStates.Normal | SelectableStates.Selected;

            var result = new ColorBlockStateConverter(mask, _written).Convert(Authored());

            Assert.AreEqual(_written, result.normalColor, "Normal sits inside the mask.");
            Assert.AreEqual(_written, result.selectedColor, "Selected sits inside the mask.");
        ...
        {
            var result = new ColorBlockStateConverter(SelectableStates.All, _written).Convert(Authored());

            foreach (var state in _everyState)
                Assert.AreEqual(_written, ColorOf(result, state), $"{state} sits inside the All mask.");
        }

        [Test]
        public void StateConverter_NoneMask_ChangesNothing()
        {
            var result = new ColorBlockStateConverter(SelectableStates.None, _written).Convert(Authored());
        ...
        // the theme had no business recoloring. The order matters — the state converter has to run
        // second, or the tint multiplies the authored color it just wrote.
        [Test]
        public void StateConverter_ChainedAfterTheTint_RestoresTheDisabledColorTheThemeTook()
        {
            var themed = new ColorBlockTintConverter(_tint).Convert(Authored());
            var result = new ColorBlockStateConverter(SelectableStates.Disabled, _written).Convert(themed);

            Assert.AreEqual(_written, result.disabledColor, "The disabled color should be the authored one, not the tinted one.");
        ...
        /// <summary>
        /// A block whose five state colors are all different, none of them gray, white or the tint.
        /// </summary>
        ...
        /// <summary>
        /// Asserts that every state the mask excludes came through with the authored color untouched.
        /// </summary>
