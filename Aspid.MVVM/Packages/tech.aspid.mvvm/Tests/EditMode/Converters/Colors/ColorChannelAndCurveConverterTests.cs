...
namespace Aspid.MVVM.Tests
{
    ...
    /// </summary>
    [TestFixture]
    public sealed class ColorChannelAndCurveConverterTests
    {
        ...
        // Set clamps the operand itself, so an HDR color cannot be authored into the operand field
        ...
        // the documented behavior, but the surprise is worth pinning: one converter can emit a
        // color that is clamped in RGB and above one in alpha at the same time.
        [Test]
        ...
        [Test]
        public void Convert_DefaultConstructed_IsAnIdentityForAnLdrColor()
        {
        ...
        // bound color through". It does for LDR only: the default clamp is on and the default mask
        // is Rgb, so a freshly picked converter silently crushes an HDR color to white while
        ...
        // The operation comes off a serialized field, so an undeclared one is a broken asset and not
        // a bad argument — reported on every push, once per written channel, with the channel left as
        // it arrived. The color is HDR, so a fall back that still ran the clamp would fail here.
        [Test]
        ...
        // The operation is read per channel and only after the mask test, so an unreachable operation
        // is never reported. A converter left in this state looks like it works.
        [Test]
        ...
        // not _to. Nothing in the class normalizes the curve.
        [TestCase(0f)]
        [TestCase(0.5f)]
        [TestCase(1f)]
        ...
        // With the clamp cleared the amount reaches Color.LerpUnclamped as it arrived, so 3 carries
        // a third of the way past _to and past one, and -1 goes below zero. A clamped implementation
        // answers 0.6 and 0.2 for the first two rows.
        [TestCase(3f, 1.4f)]
        [TestCase(-1f, -0.2f)]
        [TestCase(0.5f, 0.4f)]
        ...
        // driven past one produces a negative alpha rather than a transparent color.
        [Test]
        ...
        // Unclamped skips the curve entirely, because a curve answers with its end key past either
        // end of its own range and would clamp the amount back. A routing that ran the curve first
        // would read 0.3 for both rows — the constant curve's 0.25 of the span.
        [TestCase(0f, 0.2f)]
        [TestCase(1f, 0.6f)]
        ...
        [Test]
        public void Convert_NotInterpolating_HoldsTheStopColorUntilTheNextThreshold()
        {
            var converter = new ThresholdColorConverter(
        ...
        [Test]
        public void Convert_Interpolating_UnsortedStops_BlendsTowardTheNextUp()
        {
            var converter = new ThresholdColorConverter(
        ...
        // instead of the immediate one gives 0.75 gray here.
        [Test]
        ...
        // the converter returns it flat. The docs say blending gives "the color between that stop
        ...
        // t is zero at the lower stop, so blending does not shift the color of a value sitting
        ...
        // The span is upper - lower, not a normalized 0..1 distance, so a negative lower bound has
        ...
        // Same tie-break on the upper side, where it decides which color is blended toward:
        ...
        // A converter with no stops can never pick a color, so it reports the misconfiguration on
        // every push rather than quietly answering with the fallback for the life of the scene.
        [Test]
        public void Convert_NullStops_ReturnsTheFallbackEvenWhenInterpolating()
        {
            LogAssert.Expect(LogType.Error, new Regex("ThresholdColorConverter.*no stops are authored"));
        ...
        // channel delta instead — a blended color is arithmetic, not a copied constant.
