...
namespace Aspid.MVVM.Tests
{
    ...
    /// </summary>
    [TestFixture]
    public sealed class GradientEvaluateConverterTests
    {
        ...
        [Test]
        public void Convert_DefaultRange_ReadsTheEndsOfTheGradient()
        {
            var converter = new GradientEvaluateConverter(BlackToWhite());

            Assert.AreEqual(0f, converter.Convert(0f).r, 1e-4f);
            Assert.AreEqual(1f, converter.Convert(1f).r, 1e-4f);
        }
        ...
        [Test]
        public void Convert_Double_ReadsTheSameStop()
        {
            var gradient = BlackToWhite();
            Assert.AreEqual(
