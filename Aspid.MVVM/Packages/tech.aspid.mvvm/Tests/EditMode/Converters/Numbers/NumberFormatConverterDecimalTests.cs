...
{
    /// <summary>
    /// Coverage for <see cref="NumberFormatConverter"/> — the standard .NET format strings, the
    /// culture, the general format, and the fallback for a format .NET refuses.
    /// </summary>
    [TestFixture]
    [SetCulture("")]
    public sealed class NumberFormatConverterDecimalTests
    {
        [Test]
        public void Convert_DefaultFormat_WritesTwoDecimalsWithGrouping() =>
            Assert.AreEqual("1,234.50", new NumberFormatConverter("N2").Convert(1234.5m));
        ...
        public void Convert_DefaultFormat_RoundsToTwoDecimals() =>
            Assert.AreEqual("12,345.68", new NumberFormatConverter().Convert(12345.678m));

        [Test]
        public void Convert_FFormat_OmitsGrouping() =>
            Assert.AreEqual("1234.50", new NumberFormatConverter("F2").Convert(1234.5m));

        [Test]
        public void Convert_CurrencyFormat_AddsASymbolToTheAmount()
        {
            var text = new NumberFormatConverter("C2", CultureInfoMode.InvariantCulture).Convert(12.5m);
        ...
        public void Convert_HonoursTheCulture()
        {
            ...
            Assert.AreEqual(
                "12,345.68",
                new NumberFormatConverter("N2", CultureInfoMode.InvariantCulture).Convert(12345.678m));
        }
        ...
        public void Convert_KeepsDigitsADoubleWouldLose() =>
            Assert.AreEqual(
                ...
                new NumberFormatConverter("N0").Convert(123456789012345678901234m));
        ...
        {
            LogAssert.Expect(LogType.Error, new Regex("NumberFormatConverter.*is not a .* format"));

            Assert.AreEqual(
                (1234.5m).ToString(CultureInfo.CurrentCulture),
                new NumberFormatConverter("Q").Convert(1234.5m));
        }
        ...
        {
            var converter = new NumberFormatConverter("q");
        ...
        public void Convert_UnknownCustomFormat_PrintsItselfInsteadOfTheAmount() =>
            Assert.AreEqual("qq", new NumberFormatConverter("qq").Convert(12.5m));

        private static NumberFormatConverter GeneralFormat() =>
