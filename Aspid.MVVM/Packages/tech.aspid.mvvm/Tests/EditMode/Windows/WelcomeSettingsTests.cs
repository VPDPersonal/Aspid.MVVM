...
namespace Aspid.MVVM.Tests.EditMode.Windows
{
    /// <summary>
    /// Behavioral coverage for the Welcome tab's per-user auto-show setting: the store defaults to on, raises
    /// <c>Changed</c> only on a real change, resets to its default, and the switch built by
    /// <see cref="WelcomeSettingsUI"/> mirrors the store live.
    /// </summary>
    [TestFixture]
    internal sealed class WelcomeSettingsTests
    {
        ...
        public void BuildControls_LiveSyncsSwitchFromSettings()
        {
            ...
            var autoShow = container.Q<BaseField<bool>>();
