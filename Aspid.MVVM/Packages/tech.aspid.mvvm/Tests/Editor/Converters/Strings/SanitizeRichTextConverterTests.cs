...
{
    /// <summary>
    /// Coverage for <see cref="SanitizeRichTextConverter"/> — the two <see cref="RichTextSanitize"/>
    ...
    internal sealed class SanitizeRichTextConverterTests
    {
        ...
        // the inspector, so its defaults are a shipped behaviour rather than an implementation
        // detail: strip everything, allow nothing.
        [Test]
        public void Convert_DefaultConstructed_StripsEveryTag() =>
            Assert.AreEqual("Bob", new SanitizeRichTextConverter().Convert("<size=400%>Bob"));
        ...
        // to admit the other — otherwise whitelisting colour still breaks half the authored text.
        ...
        public void Convert_NullEntryInTheAllowedList_IsSkippedWithoutDisturbingTheRest() =>
            Assert.AreEqual("<b>x</b>", new SanitizeRichTextConverter(
                RichTextSanitize.Strip, new string[] { null, "b" }).Convert("<b>x</b>"));
        ...
        public void Convert_EmptyEntryInTheAllowedList_DoesNotMatchTheNamelessTag() =>
            Assert.AreEqual("ab", new SanitizeRichTextConverter(
        ...
        // to the field initialiser rather than storing null and throwing on the first tag.
        [Test]
        public void Convert_NullAllowedArray_KeepsTheEmptyWhitelist() =>
            Assert.AreEqual("x", new SanitizeRichTextConverter(
        ...
        public void Convert_ClosingSlashWithNoName_DoesNotReadPastTheTag() =>
            Assert.AreEqual(string.Empty, new SanitizeRichTextConverter(
        ...
        // not reach this case, and the row is here to pin the behaviour the code actually has.
        ...
        public void Convert_UnterminatedBracket_SurvivesEvenWhenStrayBracketsAreOff(
            ...
            Assert.AreEqual(value, new SanitizeRichTextConverter(
        ...
        // inner '<' and obeyed it. The exact attack the class remarks promise to stop, hidden behind
        ...
        // build that had a third mode, or a hand-edited asset. It has to fail loudly rather than
        // silently pass the markup through.
        [Test]
        public void Convert_UndeclaredMode_WithATagToSanitize_Throws() =>
            Assert.Throws<ArgumentOutOfRangeException>(
                () => new SanitizeRichTextConverter((RichTextSanitize)42).Convert("<b>x</b>"));

        // The throw sits on the sanitize branch only, so a broken mode stays invisible until a
        // string that actually needs sanitizing arrives.
        [TestCase("plain")]
        [TestCase("a < b > c")]
        public void Convert_UndeclaredMode_WithNothingToSanitize_DoesNotThrow(string value) =>
            Assert.AreEqual(value, new SanitizeRichTextConverter((RichTextSanitize)42).Convert(value));

        [Test]
        public void Convert_UndeclaredMode_AllowedTag_DoesNotThrow() =>
            Assert.AreEqual("<b>x</b>", new SanitizeRichTextConverter(
        ...
        private static SanitizeRichTextConverter Strip(params string[] allowedTags) =>
            new SanitizeRichTextConverter(RichTextSanitize.Strip, allowedTags);

        private static SanitizeRichTextConverter Escape(params string[] allowedTags) =>
            new SanitizeRichTextConverter(RichTextSanitize.Escape, allowedTags);

        private static SanitizeRichTextConverter StripNoStray() =>
            new SanitizeRichTextConverter(RichTextSanitize.Strip, null, keepStrayBrackets: false);

        private static SanitizeRichTextConverter EscapeNoStray() =>
            new SanitizeRichTextConverter(RichTextSanitize.Escape, null, keepStrayBrackets: false);
    }
}
