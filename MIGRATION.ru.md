...
| `Comparisons.Inequality` | `Comparisons.NotEqual` | Enum сериализуется порядковым номером, он не изменился |

Замените эти два имени — и всё. Сцены и префабы не затронуты.
...
- [ ] Переименовать `Values` → `Mode` и `Comparisons.Inequality` → `NotEqual` в своём коде (см. § 5.1)
