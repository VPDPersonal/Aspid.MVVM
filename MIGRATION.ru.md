...
Подсистема конвертеров перестроена: 14 конвертеров стали 148, у контракта появилось обратное направление, а слой совместимости с Unity до 2023.1 объявлен устаревшим. Сцены и префабы не затронуты — каждое переименование несёт `[MovedFrom]` или `[FormerlySerializedAs]`, а устаревшие типы по-прежнему реализуются. Работа нужна только в исходном коде.

### 5.1 Переименования (ломают компиляцию)

| Было | Стало | Примечания |
|------|-------|-----------|
| `SequenceConverters<T>` | `SequenceConverter<T>` | `[MovedFrom]`, сцены не затронуты |
| `GenericToString<T>` | `GenericToStringConverter<T>` | `[MovedFrom]`, сцены не затронуты |
| `_preConvertor` / `_postConvertor` | `_preConverter` / `_postConverter` | Включая параметры публичных конструкторов; `[FormerlySerializedAs]`, сцены не затронуты |
| `Vector2ToVector3Converter.Values`, `Vector3ToVector2Converter.Values` | `Mode` | Вложенный тип enum; сериализуется порядковый номер, он не изменился |
| `Comparisons.Inequality` | `Comparisons.NotEqual` | Порядковый номер не изменился, сцены не затронуты |

Для большинства проектов вся миграция — глобальная замена этих пяти имён.
...
Генерик-версия `ConverterExtensions.ToConvert<TFrom, TTo>` — это замена, и она не устарела.
...
- **Семейство `Vector3CombineConverter` возвращает вход без изменений** вместо исключения, когда ссылка на сцену отсутствует, и сообщает об этом один раз.
...
- [ ] Глобально переименовать пять имён конвертеров (см. § 5.1)
